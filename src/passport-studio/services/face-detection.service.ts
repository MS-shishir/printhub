...
async function initDetector(): Promise<void> {
  ...
  initPromise = (async () => {
    try {
      ...
      console.error('[FaceDetection] MediaPipe init failed:', err);
 ...
 * Returns null if no face found or on error.
 *
...
export async function detectFace(imageSrc: string): Promise<FaceDetectionResult | null> {
  try {
    ...
    if (!detectorInstance) {
      // Fallback: assume face is in center 60% of image
    ...
    return new Promise((resolve) => {
      ...
      detectorInstance.send({ image: img });
    });
  } catch (err) {
