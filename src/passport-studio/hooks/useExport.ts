...
export function useExport(): UseExportReturn {
  ...
  const downloadPNG = useCallback(async (imageUrl: string, template: PassportTemplate) => {
    await withExport(async () => {
      // Use 600 DPI HD resolution for single photo download so it looks ultra sharp on screen and print
      const w = Math.round(mmToPx(template.widthMm, 600));
      const h = Math.round(mmToPx(template.heightMm, 600));
      const printReady = await resizeImageToPrintSize(imageUrl, w, h);
      await exportSinglePhoto(printReady, template, 'png');
    });
  }, [withExport]);
  ...
  const downloadJPEG = useCallback(async (imageUrl: string, template: PassportTemplate) => {
    await withExport(async () => {
      ...
      const h = Math.round(mmToPx(template.heightMm, 600));
      const printReady = await resizeImageToPrintSize(imageUrl, w, h);
      await exportSinglePhoto(printReady, template, 'jpeg', 0.95);
    });
  }, [withExport]);
