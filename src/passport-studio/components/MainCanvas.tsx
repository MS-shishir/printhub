...
export default function MainCanvas({ editorRef }: MainCanvasProps) {
  ...
  }, [template.id, template.widthMm, template.heightMm, templateAspect]);
  ...
  const draw = useCallback(() => {
    ...
    if (isDocOutputStep) {
      ...
      // Render directly from high-resolution source with exact transform (razor-sharp)
  ...
  // ── High-Res Crop Generator (300 - 1200 DPI Ultra HD on demand) ────────
  const getCroppedDataUrl = useCallback((targetDpi: number = 300) => {
    if (!imgRef.current) return null;
    const img = imgRef.current;
    const frame = isCropPanel ? cropBox : fixedTemplateBox;
    const { x: cropX, y: cropY, width: cropW } = frame;

    const outW = Math.max(10, Math.round((template.widthMm / 25.4) * targetDpi));
    const outH = Math.max(10, Math.round((template.heightMm / 25.4) * targetDpi));
  ...
  // Debounced real-time preview sync (300 DPI lossless output)
  useEffect(() => {
    const timer = setTimeout(() => {
      const url = getCroppedDataUrl(300);
  ...
  }, [cropBox, state.transform, state.bgConfig.color, template.id, template.widthMm, template.heightMm, getCroppedDataUrl, dispatch]);
  ...
  useEffect(() => {
    let mousePosRaf: number | null = null;

    const onMouseMove = (e: MouseEvent) => {
      const mode = dragModeRef.current;

      // Update cursor coordinate display only when not actively dragging
      if (!mode && canvasRef.current) {
        if (!mousePosRaf) {
          mousePosRaf = requestAnimationFrame(() => {
