...
export default function ExportPanel() {
  ...
  const defaultTemplate = getTemplate(state.selectedTemplateId);
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-2">
        ...
        <ExportButton
          icon={FileImage}
          label="Download PNG"
          sublabel={`${activeWidthMm}×${activeHeightMm}mm · 600 DPI HD`}
          color="indigo"
      ...
      {isExporting && (
        <div className="bg-indigo-900/20 border border-indigo-700/30 rounded-lg p-3 flex items-center gap-2">
          ...
          <p className="text-[10px] text-indigo-300 font-medium">Exporting high-resolution 600 DPI image...</p>
        </div>
      )}
      ...
      <div className="border-t border-slate-700/50 pt-3 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-500">
          <div>DPI: <span className="text-slate-300">600 HD</span></div>
          <div>Format: <span className="text-slate-300">sRGB</span></div>
          <div>Width: <span className="text-slate-300">{Math.round((activeWidthMm / 25.4) * 600)}px</span></div>
          <div>Height: <span className="text-slate-300">{Math.round((activeHeightMm / 25.4) * 600)}px</span></div>
        </div>
      </div>
    </div>
  );
