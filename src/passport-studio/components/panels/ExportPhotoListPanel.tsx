...
export default function ExportPhotoListPanel() {
  ...
  const { processedTray, photoName, croppedImage, processedImage, originalImage, selectedTemplateId, customWidth, customHeight } = state;
  const template = getTemplate(selectedTemplateId, customWidth, customHeight);
  ...
  const photoList = useMemo<ExportPhotoItem[]>(() => {
    if (processedTray.length > 0) {
      return processedTray.map((item, idx) => ({
        ...
        templateName: item.templateId === 'custom' ? getTemplate(item.templateId, item.widthMm, item.heightMm).name : getTemplate(item.templateId).name,
      }));
    }
