/**
 * CropPerspectiveEngine.ts
 * Enterprise Client-Side 4-Point Homography, Perspective Warping & Classical Computer Vision Engine.
 * 
 * Features:
 * 1. 3x3 Projective Homography Matrix Solver (8x8 Gaussian Elimination with Partial Pivoting)
 * 2. High-Speed Sub-Pixel Bilinear Backward Mapping for sharp rectified document/photo outputs
 * 3. Multi-Pass Canny/Sobel Gradient Contour Analysis for 4-Corner Document Edge Detection
 * 4. Automatic Corner Sorting & Geometric Aspect-Ratio Preservation
 */

import { Point2D, DocumentQuad, Matrix3x3 } from './CropTypes';

export class CropPerspectiveEngine {
  /**
   * Calculate 3x3 Homography Projection Matrix H mapping source quad to destination quad.
   * [x', y', 1]^T = H * [x, y, 1]^T
   */
  public static getPerspectiveTransform(src: DocumentQuad, dst: DocumentQuad): Matrix3x3 {
    const P: number[][] = [];
    const pts = [
      { s: src.tl, d: dst.tl },
      { s: src.tr, d: dst.tr },
  ...
  /**
   * Invert a 3x3 Matrix for backward pixel mapping
   */
  public static invertMatrix3x3(m: Matrix3x3): Matrix3x3 | null {
    const [
      a11, a12, a13,
      a21, a22, a23,
      a31, a32, a33
  ...
  /**
   * Multi-Pass Computer Vision Document 4-Corner Detector.
   * Extracts edge gradients and raycasts from center to find boundary transitions.
   */
  public static autoDetectDocumentCorners(canvas: HTMLCanvasElement): DocumentQuad {
    ...
    const defaultQuad: DocumentQuad = {
      tl: { x: Math.round(w * 0.05), y: Math.round(h * 0.05) },
      tr: { x: Math.round(w * 0.95), y: Math.round(h * 0.05) },
      br: { x: Math.round(w * 0.95), y: Math.round(h * 0.95) },
      bl: { x: Math.round(w * 0.05), y: Math.round(h * 0.95) },
    };
    ...
    try {
      ...
      // Downscale to fast analysis proxy (320px)
      const maxDim = 320;
      ...
      if (quadArea < totalArea * 0.30 || quadArea > totalArea * 0.99) {
        return defaultQuad;
      }
