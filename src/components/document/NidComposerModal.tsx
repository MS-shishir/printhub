...
interface NidComposerModalProps {
  ...
  onAddAsNewPage?: (composedCanvas: HTMLCanvasElement, title: string) => void;
  language: 'en' | 'bn';
}
...
export default function NidComposerModal({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, side: 'front' | 'back') => {
    ...
    try {
      if (PdfEngine.isPdf(file)) {
        const rendered = await PdfEngine.renderPdfToCanvases(file, 300);
        if (rendered.length > 0) {
          const frontUrl = rendered[0].canvas.toDataURL('image/png');
          if (side === 'front') {
            setFrontImage(frontUrl);
            if (rendered.length > 1 && !backImage) {
              setBackImage(rendered[1].canvas.toDataURL('image/png'));
            }
          } else {
            setBackImage(frontUrl);
          }
        }
      } else {
      ...
      console.error('Failed to load file in NID Composer:', err);
    } finally {
      e.target.value = '';
    }
  };
  ...
  const handlePrint = () => {
    ...
    window.dispatchEvent(new CustomEvent('printhub:open-custom-print', {
      detail: {
        source: sheet.toDataURL('image/png', 1.0),
        title: `NID_PrintSheet_${cardType}`,
      }
    }));
  };
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <input
                    ref={frontInputRef}
                    type="file"
                    accept="image/*,application/pdf,.pdf,.jpg,.jpeg,.png,.webp,.bmp"
                    className="hidden"
