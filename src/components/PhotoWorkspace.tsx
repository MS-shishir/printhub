...
import PuppetWarpModal from './photo/PuppetWarpModal';
import SideReconstructionModal from './photo/SideReconstructionModal';
...
export default function PhotoWorkspace({ onAddRecentFile, language }: PhotoWorkspaceProps) {
  ...
  const [isPuppetWarpModalOpen, setIsPuppetWarpModalOpen] = useState<boolean>(false);
  const [puppetWarpSource, setPuppetWarpSource] = useState<HTMLCanvasElement | HTMLImageElement | null>(null);
  const [isSideRepairModalOpen, setIsSideRepairModalOpen] = useState<boolean>(false);
  const [sideRepairSource, setSideRepairSource] = useState<HTMLCanvasElement | HTMLImageElement | null>(null);
  ...
  const handleSelectTool = (tool: ToolType) => {
    ...
    if (tool === 'move' || tool === 'select') {
      ...
      if (tool === 'clone') handleDuplicateActiveObject();
        ...
        setCropMode(tool === 'crop' ? 'normal' : 'perspective');
        setIsCropActive(true);
      }
      else if (tool === 'puppet_warp') {
        const canvas = fabricCanvasRef.current;
        if (canvas) {
          const activeObj = canvas.getActiveObject() || canvas.getObjects().find((o) => o.isType('image'));
          let srcEl: HTMLCanvasElement | HTMLImageElement | null = null;
          if (activeObj && activeObj.isType('image')) {
            srcEl = (activeObj as any)._rawSourceElement || (activeObj as fabric.Image).getElement();
          }
          if (!srcEl) {
            const snapshot = document.createElement('canvas');
            snapshot.width = canvas.width || 800;
            snapshot.height = canvas.height || 600;
          ...
          setPuppetWarpSource(srcEl);
          setIsPuppetWarpModalOpen(true);
        }
      }
      else if (tool === 'side_repair') {
        handleOpenSideRepair();
      }
  ...
  const handleApplyPuppetWarp = (deformedCanvas: HTMLCanvasElement) => {
    if (!fabricCanvasRef.current) return;
    const dataUrl = deformedCanvas.toDataURL('image/png', 1.0);
    const imgObj = new Image();
    imgObj.onload = () => {
      if (!fabricCanvasRef.current) return;
      const fabricImage = new fabric.Image(imgObj);
      (fabricImage as any)._rawSourceElement = deformedCanvas;
      (fabricImage as any).id = `deform-${Date.now()}`;
      (fabricImage as any).name = 'Deformed Photo';
      ...
      if (activeObj) {
        ...
        if (existing) {
          ...
          fabricCanvasRef.current.remove(existing);
        }
      }
      ...
      showToast(
        language === 'bn'
          ? 'মেশ ডিফর্মেশন ও মাথা সোজা সফলভাবে সম্পন্ন হয়েছে!'
          : 'Mesh deformation & head alignment applied!',
        'success'
      );
    };
    imgObj.src = dataUrl;
  ...
  const handleOpenSideRepair = () => {
    ...
    if (canvas) {
      ...
      setSideRepairSource(srcEl);
      setIsSideRepairModalOpen(true);
    }
  };

  const handleApplySideRepair = (reconstructedCanvas: HTMLCanvasElement) => {
    if (!fabricCanvasRef.current) return;
    const dataUrl = reconstructedCanvas.toDataURL('image/png', 1.0);
    const imgObj = new Image();
    imgObj.onload = () => {
      if (!fabricCanvasRef.current) return;
      const fabricImage = new fabric.Image(imgObj);
      (fabricImage as any)._rawSourceElement = reconstructedCanvas;
      (fabricImage as any).id = `recon-${Date.now()}`;
      (fabricImage as any).name = 'Reconstructed Portrait';
      ...
      showToast(
        language === 'bn'
          ? 'ম্যানুয়াল সাইড রিকনস্ট্রাকশন সফলভাবে ক্যানভাসে যুক্ত হয়েছে'
          : 'Geometric side reconstruction applied successfully',
        'success'
      );
    };
    imgObj.src = dataUrl;
  ...
  const handleConfirmExport = async (
    ...
    quality: number,
    transparent: boolean,
    exportScope: 'photo' | 'canvas' = 'photo'
  ) => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    // Save previous viewport, zoom, background and active selection
    const prevVpt = (canvas.viewportTransform ? [...canvas.viewportTransform] : [1, 0, 0, 1, 0, 0]) as fabric.TMat2D;
    const prevZoom = canvas.getZoom();
    const prevBg = canvas.backgroundColor;
    const activeObj = canvas.getActiveObject();

    // Deselect object so selection handles aren't exported
    canvas.discardActiveObject();
    // Temporarily reset viewport & zoom to 1:1 pristine canvas coordinates
    ...
    try {
      let cropLeft = 0;
      let cropTop = 0;
      let cropWidth = canvas.width || 800;
      let cropHeight = canvas.height || 600;

      const objects = canvas.getObjects().filter((o) => o.visible !== false);

      if (exportScope === 'photo' && objects.length > 0) {
        let minX = Infinity;
        let minY = Infinity;
        let maxX = -Infinity;
        let maxY = -Infinity;
        ...
        if (minX !== Infinity && minY !== Infinity && maxX > minX && maxY > minY) {
          cropLeft = Math.max(0, Math.floor(minX));
          cropTop = Math.max(0, Math.floor(minY));
          cropWidth = Math.min(canvas.width || 1200, Math.ceil(maxX)) - cropLeft;
          cropHeight = Math.min(canvas.height || 800, Math.ceil(maxY)) - cropTop;
        }
      }

      // Multiplier to preserve original image resolution
      let multiplier = 1;
      const targetImage = objects.find((o) => o.isType('image')) as fabric.Image | undefined;
      if (targetImage) {
        const el = (targetImage as any)._rawSourceElement || targetImage.getElement();
        const origW = el?.naturalWidth || el?.width || 0;
        const scaledW = targetImage.getScaledWidth();
        if (origW > 0 && scaledW > 0) {
          const naturalRatio = origW / scaledW;
          multiplier = Math.max(1, Math.min(4, naturalRatio * (quality / 100)));
        } else {
          multiplier = Math.max(1, (quality / 100) * 2);
        }
      } else {
        multiplier = Math.max(1, (quality / 100) * 2);
      }

      const cleanBaseName = photoName ? photoName.replace(/\.[^/.]+$/, '') : 'Photo_Studio';

      if (format === 'SVG') {
        const svgString = canvas.toSVG({
          viewBox: { x: cropLeft, y: cropTop, width: cropWidth, height: cropHeight }
        });
        const blob = new Blob([svgString], { type: 'image/svg+xml;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `${cleanBaseName}_edited.svg`;
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `${cleanBaseName}_edited.pdf`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
      } else {
        const mimeFormat = format === 'PNG' ? 'png' : format === 'JPG' ? 'jpeg' : 'webp';
        const dataUrl = canvas.toDataURL({
          format: mimeFormat,
          quality: quality / 100,
          left: cropLeft,
        ...
        const link = document.createElement('a');
        link.href = dataUrl;
        link.download = `${cleanBaseName}_edited.${format.toLowerCase()}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      }
      ...
      showToast(
        language === 'bn' ? 'ফটো সফলভাবে ডাউনলোড হয়েছে!' : 'Photo exported successfully!',
        'success',
        language === 'bn' ? 'ডাউনলোড সম্পন্ন' : 'Export Complete'
      );
    } catch (err) {
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden bg-slate-950 text-slate-100 select-none">
      ...
      <PhotoToolbar
        ...
        onOpenSideRepair={handleOpenSideRepair}
      ...
      {/* 2D Moving Least Squares Puppet Warp & Head Straightener Modal */}
      {isPuppetWarpModalOpen && (
        <PuppetWarpModal
          isOpen={isPuppetWarpModalOpen}
          onClose={() => setIsPuppetWarpModalOpen(false)}
          sourceCanvasOrImage={puppetWarpSource}
          onApply={handleApplyPuppetWarp}
          language={language}
        />
      )}

      {/* Manual Geometric Side Reconstruction & Repair Modal */}
      {isSideRepairModalOpen && (
        <SideReconstructionModal
          isOpen={isSideRepairModalOpen}
          onClose={() => setIsSideRepairModalOpen(false)}
          sourceCanvasOrImage={sideRepairSource}
          onApply={handleApplySideRepair}
          language={language}
        />
      )}
    </div>
